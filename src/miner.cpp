...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("CONVIERGEMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("CONVIERGEMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("CONVIERGEMiner started\n");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running CONVIERGEMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
