...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("CNNVIERGEMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("CNNVIERGEMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("CNNVIERGEMiner started\n");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running CNNVIERGEMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
